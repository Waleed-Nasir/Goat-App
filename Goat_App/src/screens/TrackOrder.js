...
const TrackOrder = () => {
  ...
  useEffect(() => {
    if (OrderDetails) {
      setCurrentIndex(TYPES[OrderDetails?.status || 3]);
    }
  }, [OrderDetails]);
  ...
  const data = [
    {
      ...
      text: "waiting", // 'Your order has been\nconfirmed!',
      title: "Confirmed",
    },
    {
    ...
    },
    {
      image:
        OrderDetails?.status === "cancelled"
          ? Assets.Goat_CrossGreen
          : Assets.Goat_TrackOrderEnd,
      text:
        OrderDetails?.status === "cancelled"
  ...
  const HandleOrderConform = () => {
    ...
    fetch(
      "https://api.goatpure.com/api/orders/mark-as-cancelled",
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        if (result.success) {
          MessageShow("success", 'Query submitted until its resolve order in cancalled state');
          dispatch(getOrders());
          navigation.goBack();
        } else {
          MessageShow("success", result.message);
        }
      })
          ...
          OrderDetails?.status === "pending" ? (
            <>
              ...
              <Button
                ...
                onPress={() => {
                  HandleOrderConform();
                  // navigation.navigate(SCREENS.SupportRequest);
                }}
              />
            </>
          ) : null
