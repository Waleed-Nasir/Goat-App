...
const CheckOut = () => {
  ...
  let SelectedCurrent = LocalAddress?.filter((item) => item?.selected)[0];
      ...
      <View style={styles.padding20}>
        {SelectedCurrent ? (
          ...
          <Address
            {...{
              Street: "Add Address",
              Nearest: "Please click arrow to add",
              City: "",
              Tag: "",
            }}
            LeftPress={() => navigation.navigate(SCREENS.SelectAddress)}
        ...
        <Button
          ...
          onPress={() => {
            if (!SelectedCurrent) {
              MessageShow("success", "Please add Address first");
              return false;
            }
            if (!accessToken) {
              MessageShow("success", "Please login to first");
              return false;
            }
            dispatch(
              getPlaceOrder({
                data: {
                  ...
                  addressLine1:
                    SelectedCurrent?.Street +
                    " " +
                    SelectedCurrent?.House +
                    " " +
                    SelectedCurrent?.Nearest +
                    " " +
